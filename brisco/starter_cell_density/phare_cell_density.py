...
def calculate_neighbourhood(
    distance_radii, kdtree, cells, cdf=cdf, bylayer=False, area_acronym="VISp"
):
    neighbours = dict()
    distance = dict()
    if bylayer:
    ...
    for layer in layers:
        ...
        neighbours[layer] = (
            np.vstack(
                [
                    kdtree.query_radius(vlc_um, r, count_only=True)
                    for r in distance_radii
                ]
            )
            - 1
        )
...
for mouse, m_df in mice_df[mice_df["Virus Batch"] == "A87"].iterrows():
    ...
    if PLOT_DENSITY:
        ...
        neighbours, distance = calculate_neighbourhood(
            distance_radii, kdtree=ctx_tree, cells=cells
        )
        fig.clear()
        ax0 = fig.add_subplot(2, 3, 1)
        ax0.set_title(
            "Total: %d cells, Cortex %d cells, V1: %d cells"
        ...
        for layer, nei in neighbours.items():
            ...
            if layer == "all":
                ...
                ax4.plot(distance_radii[beg_density:], m, label=layer, lw=3, zorder=3)
                ax4.fill_between(
                    distance_radii[beg_density:],
                    m - std,
                    m + std,
                    color="grey",
                    alpha=0.4,
        ...
        if NAPARI:
            ...
            viewer.add_points(
                cells, name="Cells", face_color="blue", opacity=0.2, size=5
            )
            viewer.add_points(
                cells[cell_in_v1],
                name="V1 Cells",
...
if PLOT_SUMMARY:
    ...
    sum_df = hypo_df.groupby(
        ["what", "mouse", "dilution", "zres"], as_index=False
    ).aggregate(np.sum)
    ...
    ax2.clear()
    sns.stripplot(
        data=count_df[count_df.area.isin(areas_to_keep)],
        x="area",
        y="count",
        ax=ax2,
        hue="zres",
    ...
    sns.stripplot(
        data=av[av.area.isin(areas_to_keep)],
        x="area",
        y="count",
        order=areas_to_keep,
        color="k",
    ...
    ax3.clear()
    sns.stripplot(
        data=density_df[density_df.area.isin(areas_to_keep)],
        x="area",
        y="density",
        ax=ax3,
        hue="zres",
    ...
    for parent_area in areas_of_interest:
        ...
        df = (
            df.groupby(["mouse", "dilution", "zres", "what"])
            .aggregate(np.sum)
            .reset_index()
        )
        df["area"] = parent_area
    ...
    if mean_only:
        good_areas = (
            good_areas.groupby(["dilution", "area"]).aggregate(np.mean).reset_index()
        )
