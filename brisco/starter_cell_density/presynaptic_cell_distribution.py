...
for mouse in mice:
    ...
    if (not REDO) and summary_density_mouse_path.is_file():
        ...
        cell_count = atlas_utils.cell_density_by_areas(
            atlas=atlas,
            atlas_id=atlas_id,
            bg_atlas=bg_atlas,
            cortex_df=cdf,
...
sns.stripplot(
    ax=ax, data=long_df, x="area", y="count", hue="mouse", order=av.index[-35:]
)
ax.set_xticklabels([])
ax = fig.add_subplot(2, 2, 3)
sns.stripplot(
    ax=ax, data=long_df, x="area", y="count", hue="mouse", order=av.index[-15:]
)
ax.set_xticklabels(
    [bg_atlas.structures[l._text]["name"] for l in ax.get_xticklabels()], rotation=90
)
...
sns.stripplot(
    ax=ax, data=long_df, x="area", y="density", hue="mouse", order=av.index[-15:]
)
ax.set_xticklabels([])
ax.get_legend().remove()
