...
out = find_cell_in_injection_site(
    project,
    mouse,
    channel,
    pixel_size_um,
    cell_scale_um=data_size_um,
    inj_min_width=100,
    inj_vol_percentile=99.99,
    cell_type="Cells",
    cell_kwargs=None,
    view_in_napari=NAPARI,
)
...
fig.savefig(
    pathlib.Path(flz.PARAMETERS["data_root"]["processed"])
    / project
    / mouse
    / "cell_vs_distance.svg",
    dpi=600,
)
fig.savefig(
    pathlib.Path(flz.PARAMETERS["data_root"]["processed"])
...
print(
    "If we assume that there are 100,000 cells/mm^3, we stained %.3f %% of the cells"
    % (len(cell_in_inj) / inj_vol / 100000)
)
msg = "There are %d cells < %.1f mm from the injection site." % (
    len(cells_local),
    dst_threshold,
)
